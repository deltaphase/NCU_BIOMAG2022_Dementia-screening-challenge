...
x = epochs.get_data(picks=alpha_channels) * 1e+15
...
IF_M = np.squeeze(IF_temp.mean(axis = (0, 1, 2))).round(2)
IF_M
...
IF_std = np.squeeze(IF_temp.std(axis = (0, 1, 2))).round(2)
IF_std
...
config['mask_amp_mode'] = 'ratio_sig'
config['imf_opts/sd_thresh'] = 0.05
imf = emd.sift.mask_sift(x[0][0], **config)
IP, IF, IA = emd.spectra.frequency_transform(imf, sample_rate, 'nht')
...
emd.plotting.plot_imfs(IF_temp[0,0,:,:], scale_y=True, cmap=True)
...
#ag_picks = mne.pick_channels(raw.ch_names, ag_names)
raw.plot_psd(0, 70, picks = alpha_channels)


# In[36]:


get_ipython().run_line_magic('pinfo', 'epochs.get_data')
...
raw=mne.io.read_raw_fif('de_hokuto_control66-raw.fif')
# find alpha
freqs_sig = 9, 12
freqs_noise = 8, 13
...
get_ipython().run_line_magic('pinfo', 'emd.plotting.plot_imfs')
...
emd.plotting.plot_imfs(IF, scale_y=True, cmap=True)


# In[40]:


IF.mean(axis=0).round(2)
